...
#ifdef USE_FLASH_ATTENTION_HOPPER
...
#include <ATen/native/transformers/cuda/flash_attn_hopper/flash.h>
#include <ATen/native/transformers/cuda/flash_attn_hopper/flash_api.h>
#include <ATen/native/transformers/cuda/flash_attn_hopper/static_switch.h>
...
void set_params_fprop(Flash_fwd_params &params,
    ...
    params.is_e4m3 = q.dtype() == at::kFloat8_e4m3fn;
...
// return {out, q_padded, k_padded, v_padded, softmax_lse};
...
mha_fwd(at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
    ...
    int const alignment = q_type == at::kFloat8_e4m3fn ? 16 : 8;
    ...
    if (out_.has_value()) {
        ...
        if (head_size_og % alignment != 0) { out = at::empty(q_padded, opts.dtype(out_type)); }
    } else {
        out = at::empty(q_padded, opts.dtype(out_type));
    }
    ...
    auto tile_count_semaphore = (params.is_causal || params.is_local) ? at::zeros({1}, opts.dtype(at::kInt)) : at::empty({1}, opts.dtype(at::kInt));
...
mha_varlen_fwd(at::Tensor &q,         // batch_size x seqlen_q x num_heads x head_size
    ...
    if (seqused_q_.has_value()){
        ...
        TORCH_CHECK(seqused_q.dtype() == at::kInt, "seqused_q must have dtype int32");
    ...
    if (seqused_k_.has_value()){
        ...
        TORCH_CHECK(seqused_k.dtype() == at::kInt, "seqused_k must have dtype int32");
    ...
    auto tile_count_semaphore = at::zeros({1}, opts.dtype(at::kInt));
    params.tile_count_semaphore = tile_count_semaphore.data_ptr<int>();
...
mha_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x head_size_og
    ...
    auto q_type = q.dtype();
    TORCH_CHECK(q_type == at::kHalf || q_type == at::kBFloat16,
    ...
    auto softmax_lse_log2 = at::empty({batch_size, num_heads, seqlen_q_rounded}, opts.dtype(at::kFloat));
    at::Tensor dq_accum;
    at::Tensor dk_accum, dv_accum;
    dq_accum = at::empty({batch_size, num_heads, seqlen_q_rounded, head_size_rounded}, opts.dtype(at::kFloat));
    if (num_heads_k != num_heads) {  // MQA / GQA
        dk_accum = at::zeros({batch_size, num_heads_k, seqlen_k_rounded, head_size_rounded}, opts.dtype(at::kFloat));
        dv_accum = at::zeros({batch_size, num_heads_k, seqlen_k_rounded, head_size_rounded}, opts.dtype(at::kFloat));
    }
    ...
    at::Tensor dq_semaphore = at::empty({(seqlen_q + kBlockM - 1) / kBlockM, batch_size, num_heads}, opts.dtype(torch::kInt32));
    params.dq_semaphore = dq_semaphore.data_ptr<int>();
    // printf("dq_semaphore: %p, [%d, %d, %d]\n", params.dq_semaphore, (seqlen_q + 64 - 1) / 64, batch_size, num_heads);
    if (num_heads_k != num_heads) {
        at::Tensor dk_semaphore = at::zeros({(seqlen_k + kBlockN - 1) / kBlockN, batch_size, num_heads_k}, opts.dtype(torch::kInt32));
        at::Tensor dv_semaphore = at::zeros({(seqlen_k + kBlockN - 1) / kBlockN, batch_size, num_heads_k}, opts.dtype(torch::kInt32));
...
mha_varlen_bwd(const at::Tensor &dout,  // batch_size x seqlen_q x num_heads, x head_size_og
    ...
    auto softmax_d = at::empty({num_heads, total_q_padded_rounded}, opts.dtype(at::kFloat));
    auto softmax_lse_log2 = at::empty({num_heads, total_q_padded_rounded}, opts.dtype(at::kFloat));
    at::Tensor dq_accum;
    at::Tensor dk_accum, dv_accum;
    dq_accum = at::empty({num_heads, total_q_padded_rounded, head_size_rounded}, opts.dtype(at::kFloat));
    if (num_heads_k != num_heads) {  // MQA / GQA
        dk_accum = at::zeros({num_heads_k, total_k_padded_rounded, head_size_rounded}, opts.dtype(at::kFloat));
        dv_accum = at::zeros({num_heads_k, total_k_padded_rounded, head_size_rounded}, opts.dtype(at::kFloat));
    }
    ...
    // Will be zero'ed out in the backward preprocess kernel
    at::Tensor dq_semaphore = at::empty({(max_seqlen_q + kBlockM - 1) / kBlockM, batch_size, num_heads}, opts.dtype(torch::kInt32));
    params.dq_semaphore = dq_semaphore.data_ptr<int>();
    if (num_heads_k != num_heads) {
        // TODO: do we need to zero them out?
        at::Tensor dk_semaphore = at::empty({(max_seqlen_k + kBlockN - 1) / kBlockN, batch_size, num_heads_k}, opts.dtype(torch::kInt32));
        at::Tensor dv_semaphore = at::empty({(max_seqlen_k + kBlockN - 1) / kBlockN, batch_size, num_heads_k}, opts.dtype(torch::kInt32));
        params.dk_semaphore = dk_semaphore.data_ptr<int>();
        params.dv_semaphore = dv_semaphore.data_ptr<int>();
    }
