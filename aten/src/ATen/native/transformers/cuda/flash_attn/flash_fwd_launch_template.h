...
#include <ATen/native/transformers/cuda/flash_attn/tile_scheduler.hpp>
...
#include <ATen/native/transformers/cuda/flash_attn/seq_len.h>
...
void run_flash_fwd(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    using CollectiveMainloop = pytorch_flash::CollectiveMainloopFwd<Kernel_traits, Is_causal, Seqlen_traits>;
    using CollectiveEpilogue = pytorch_flash::CollectiveEpilogueFwd<Kernel_traits, Seqlen_traits>;
    using Scheduler = std::conditional_t<
        Seqlen_traits::kUseVarSeqLen, 
        pytorch_flash::SingleTileScheduler,
        std::conditional_t<!Is_causal,
            pytorch_flash::StaticPersistentTileScheduler,
            pytorch_flash::DynamicPersistentTileScheduler<Kernel_traits::kNThreads - cutlass::NumThreadsPerWarpGroup, Kernel_traits::NumProducerThreads>
    >>;
    // using Scheduler = pytorch_flash::SingleTileScheduler;
    ...
    if constexpr(cutlass::sizeof_bits_v<Element> == 8)
        kernel = (void *)pytorch_flash::compute_attn_ws_fp8<Kernel_traits, Is_causal, Scheduler, Seqlen_traits>;
    else
        kernel = (void *)pytorch_flash::compute_attn_ws<Kernel_traits, Is_causal, Scheduler, Seqlen_traits>;
...
void run_mha_fwd_hdim64_fp8(Flash_fwd_params &params, cudaStream_t stream) {
    ...
    using Seqlen_traits = pytorch_flash::FixedSeqLenTraits;
    if(params.is_causal) {
