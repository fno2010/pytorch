...
public:
    ...
    operator()(Params const& params, [[maybe_unused]] char* smem_buf) {
        ...
        Tensor o_fp32 = pytorch_flash::convert_type<float>(make_tensor(tOrO.data(), l));
        Tensor do_fp32 = pytorch_flash::convert_type<float>(make_tensor(tOrdO.data(), l));
        ...
        for (int mi = 0; mi < size<0>(o_fp32); ++mi) {
            ...
            dP_sum(mi) = pytorch_flash::Allreduce<kGmemThreadsPerRow>::run(dP_sum_cur, sum_op);
        }
