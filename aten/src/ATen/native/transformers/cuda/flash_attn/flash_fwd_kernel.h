...
#include <ATen/native/transformers/cuda/flash_attn/mainloop_fwd_sm90_tma_gmma_ws.hpp>
#include <ATen/native/transformers/cuda/flash_attn/epilogue_fwd_sm90_tma.hpp>
...
__global__ void __launch_bounds__(Ktraits::kNWarps * cutlass::NumThreadsPerWarp, 1)
    ...
    if (warp_group_idx == 0) {  // Producer
        ...
        for (auto work_tile_info = scheduler.get_initial_work();
            ...
            pytorch_flash::Softmax<2 * (2 * kBlockM / NumMmaThreads)> softmax(mainloop_params.softmax_scale_log2);
...
__global__ void __launch_bounds__(Ktraits::kNWarps * cutlass::NumThreadsPerWarp, 1)
    ...
    if (warp_group_idx == 0) {  // Producer
        ...
        for (auto work_tile_info = scheduler.get_initial_work();
            ...
            // Attention output (GEMM-II) accumulator.
            Tensor tOrO = partition_fragment_C(tiled_mma1, select<0, 2>(TileShape_MNK{}));
            pytorch_flash::Softmax<2 * (2 * kBlockM / NumMmaThreads), Use_max_offset> softmax(shared_storage.softmax_scale_qk_log2);
